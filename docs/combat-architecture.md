...
Rust `context.combat:forecast` receives two compact participants once. It merges states by both HP values, slowed flags, combat termination and whether either participant has been hit. It returns marginal HP distributions, expected HP, death probability and probability of no hits. Forecasts do not consume RNG or mutate the world. The arithmetic of damage and healing is shared with actual strikes through `context.combat:impact`; the healing percentage and constant come from Lua. Actual strike calls transfer scalar values, never unit objects. Lua supplies an explicit strike sequence, including full berserk exchanges with unequal strike counts and firststrike. The kernel executes this sequence without interpreting a berserk rule. Healing also accepts a Lua-supplied constant and uses a Lua-supplied lower HP bound for negative healing.
...
Detailed upstream comparison and remaining gaps: [combat-parity.md](combat-parity.md). This architecture does not imply complete WML/WFL compatibility.
